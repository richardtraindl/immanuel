...
def rank_moves(priomoves):
    for pmove in priomoves:
        ...
        if(token & MV_IS_CAPTURE > 0):
            # capturing piece is pawn or captured piece if officer
            if(token & MV_PIECE_IS_PAWN > 0 or token & CAPTURED_IS_PAWN == 0):
                #count += 1
                pmove[3] = min(PRIO['prio1'], pmove[3])
