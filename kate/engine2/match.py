...
class cMatch:
    ...
    def __init__(self):
        ...
        self.time_start = 0
        self.white_player = cPlayer(COLORS['white'], "Immanuel", False, 0)
    ...
    def is_move_after_move_valid(self, srcx, srcy, dstx, dsty, prom_piece):
        if(not self.is_move_inbounds(srcx, srcy, dstx, dsty)):
            return False, self.RETURN_CODES['out-of-bounds']
        piece = self.board.readfield(srcx, srcy)
        cpiece = self.obj_for_piece(piece, srcx, srcy)
    ...
    def do_move(self, srcx, srcy, dstx, dsty, prom_piece):
        ...
        cpiece = self.obj_for_piece(piece, srcx, srcy)
        if(cpiece):
            move = cpiece.do_move(dstx, dsty, prom_piece)
